...
def isvalid_card_number(card_number):

    if '-' in card_number and not re.match(r"(\d{4}-){3}\d{4}$", card_number):
        return "Invalid"
    ...
    if not re.match(r"[456][0-9]{15}$", card_number):        
        return "Invalid"
...
for _ in range(int(input())):
    card_number = input()
    print(isvalid_card_number(card_number))
